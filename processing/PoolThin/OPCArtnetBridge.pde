...
public class OPCArtnetBridge implements Runnable
{
  //Thread thread;
  Socket socket;
  ArtNetClient artnet;
  ...
  OPCArtnetBridge(PApplet parent, String host, boolean showLocations)
  {
    this.host = host;
   ...
   * Angle is in radians, measured clockwise from +X.
   **/
  void ledRay(int index, int count, float x, float y, float offset, float spacing, float angle)
  {
    float s = sin(angle);
    float c = cos(angle);
    for (int i = 0; i < count; i++) {
   ...
   * Angle is in radians, measured clockwise from +X.
   **/
  void ledRayLayout(int index, int ray, LayoutLoader layout, int ledCount)
  {
